...
export default async function handler(req, res) {
    ...
    try {
        ...
        const referer = isCoflixRelated ? 'https://coflix.band/' : (urlObj.origin + '/');
        const origin = isCoflixRelated ? 'https://coflix.band' : urlObj.origin;
        ...
        if (isCoflixRelated) {
            ...
            if (cachedCookies && (now - lastCookieFetch < 15 * 60 * 1000)) {
                ...
                try {
                    ...
                    const warmRes = await axios.get('https://coflix.band/', { 
