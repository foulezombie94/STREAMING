...
function updateTvIframe() {
    ...
    // Utilisation de tous les paramètres: ds_lang=fr (sous-titres FR), autoplay=1 (lecture auto), autonext=1 (épisode suivant automatique)
    videoIframe.src = `https://vidsrc-embed.ru/embed/tv?tmdb=${mediaId}&season=${s}&episode=${e}&ds_lang=fr&autoplay=1&autonext=1`;
}
...
if (watchMovieBtn && playerSection && videoIframe) {
    watchMovieBtn.addEventListener('click', () => {
        ...
        if (mediaType === 'movie') {
            ...
            // Utilisation des paramètres: ds_lang=fr et autoplay=1
            videoIframe.src = `https://vidsrc-embed.ru/embed/movie?tmdb=${mediaId}&ds_lang=fr&autoplay=1`;
        } else {
